...
## vMB: dup metrics file written to output folder.
...
cmd=(/usr/lib/jvm/java-1.7.0-openjdk-amd64/bin/java -Xmx16G -jar /scratch/Tools/GATK/GenomeAnalysisTK.jar -S SILENT -I $dedupBAM -R $REF -T RealignerTargetCreator -o $outFOLDER'/'target.intervals -nt $ncores)
echo "${cmd[@]}"
"${cmd[@]}"

# realign BAM around indels > STEP2 <
realBAM=$outFOLDER'/'$(basename "$dedupBAM" .bam).realigned.bam
cmd=(/usr/lib/jvm/java-1.7.0-openjdk-amd64/bin/java -Xmx16G -jar /scratch/Tools/GATK/GenomeAnalysisTK.jar -S SILENT -I $dedupBAM -R $REF -T IndelRealigner -targetIntervals $outFOLDER'/'target.intervals -o $realBAM)
echo "${cmd[@]}"
"${cmd[@]}"
